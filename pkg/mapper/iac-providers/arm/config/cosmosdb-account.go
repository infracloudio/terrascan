...
func CosmosDBAccountConfig(r types.Resource, params map[string]interface{}) interface{} {
	...
	var v string
	ipr := convert.ToSlice(r.Properties, ipRules)
	for _, s := range ipr {
		m := s.(map[string]interface{})
		v = convert.ToString(m, ipAddressOrRange)
		break
	}
	if v != "" {
		cf.IPRangeFilter = v
